...
pub trait IntoTransform2<G: Gain> {
    /// transform gain data
    ///
    /// # Arguments
    ///
    ...
    ///
    fn with_transform2<FT: Fn(&Transducer, Drive) -> Drive, F: Fn(&Device) -> FT>(
        self,
        f: F,
    ) -> Transform2<G, FT, F>;
}

impl<G: Gain, FT: Fn(&Transducer, Drive) -> Drive, F: Fn(&Device) -> FT> Transform2<G, FT, F> {
    #[doc(hidden)]
...
mod tests {
    ...
    fn test() -> anyhow::Result<()> {
        ...
        let gain = TestGain::null().with_transform2(move |_| move |_, _| d);
