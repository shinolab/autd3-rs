        ...
        geometry.iter().for_each(|dev| {
            if let Some(key) = key_map(dev) {
                if let Some(v) = filters.get_mut(&key) {
                    ...
                    filters.insert(key, DeviceFilter::from_fn(num_devices, |i| i == dev.idx()));
                }
            }
        });
        filters
            ...
            .try_for_each(|(k, filter)| -> Result<(), AUTDDriverError> {
                {
                    ...
                    operations
                        .iter_mut()
                        .zip(geometry.iter())
                        .filter(|(_, dev)| filter.is_enabled(dev.idx()))
                        .for_each(|(op, dev)| {
    ...
    fn test_group() -> anyhow::Result<()> {
        ...
        impl Datagram for TestDatagram {
            ...
            fn operation_generator(
                ...
                geometry.iter().for_each(|dev| {
                    self.test.lock().unwrap()[dev.idx()] = filter.is_enabled(dev.idx());
                });
                Ok(NullOperationGenerator)
            }
        }
