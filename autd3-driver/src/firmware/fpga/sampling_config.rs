...
impl SamplingConfig {
    ...
    pub fn division(&self, ultrasound_freq: u32) -> Result<u32, AUTDInternalError> {
        ...
        match *self {
            ...
            Self::Freq(f) => {
                if (ultrasound_freq % f.hz()) != 0 {
                    return Err(AUTDInternalError::SamplingFreqInvalid(f, ultrasound_freq));
                }
                Self::division_from_freq_nearest(f.hz() as _, base_freq)
            }
            Self::FreqNearest(f) => Self::division_from_freq_nearest(f.hz(), base_freq),
        }
    }
...
mod tests {
    ...
    #[rstest::rstest]
    #[test]
    #[case::min(Ok(20480000), freq_min()*Hz, 40000)]
    #[case::max(Ok(512), freq_max(20480000)*Hz, 40000)]
    #[case(Err(AUTDInternalError::SamplingFreqInvalid(512*Hz, 40000)), 512*Hz, 40000)]
    #[case::not_supported_max(
        Err(AUTDInternalError::SamplingFreqInvalid(
            (freq_max(20480000) - 1)*Hz,
            40000
        )),
        (freq_max(20480000) - 1)*Hz,
        40000
    ...
    #[case::out_of_range_max(
        Err(AUTDInternalError::SamplingFreqInvalid(
            (freq_max(20480000) * 2)*Hz,
            40000
        )),
        (freq_max(20480000) * 2)*Hz,
        40000
    )]
    #[case::min(Ok(80000*512), freq_min()*Hz, 80000)]
    #[case::max(Ok(512), freq_max(80000*512)*Hz, 80000)]
    #[case(Err(AUTDInternalError::SamplingFreqInvalid(512*Hz, 80000)), 512*Hz, 80000)]
    #[case::not_supported_max(
        Err(AUTDInternalError::SamplingFreqInvalid(
            (freq_max(80000*512) - 1)*Hz,
            80000
        )),
        (freq_max(80000*512) - 1)*Hz,
        80000
    ...
    #[rstest::rstest]
    #[test]
    #[case::min(Ok(0xFFFFFFFF), freq_min_raw(20480000)*Hz, 40000)]
    #[case::max(Ok(512), freq_max_raw(20480000)*Hz, 40000)]
    #[case(Ok(40000), 512.*Hz, 40000)]
    #[case::not_supported_max(
        Ok(512),
        (freq_max(20480000) as f64 - 1.)*Hz,
        40000
    )]
    #[case::out_of_range_min(
        ...
        (freq_min_raw(20480000) - f64::MIN)*Hz,
        40000
    )]
    #[case::out_of_range_max(
        Err(AUTDInternalError::SamplingFreqOutOfRange(
        ...
        (freq_max_raw(20480000) + f64::MIN)*Hz,
        40000
    )]
    #[case::min(Ok(0xFFFFFFFF), freq_min_raw(80000*512)*Hz, 80000)]
    #[case::max(Ok(512), freq_max_raw(80000*512)*Hz, 80000)]
    #[case(Ok(80000), 512.*Hz, 80000)]
    #[case::not_supported_max(
        Ok(512),
        (freq_max(80000*512) as f64 - 1.)*Hz,
        80000
    )]
    #[case::out_of_range_min(
        Err(AUTDInternalError::SamplingFreqOutOfRange(
    ...
    #[case::out_of_range_max(
        ...
        (freq_max_raw(80000*512) + f64::MIN)*Hz,
        80000
    )]
