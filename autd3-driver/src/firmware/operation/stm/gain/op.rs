...
pub struct GainSTMOp<
    'a,
    F: ExactSizeIterator<Item = Box<dyn Fn(&Transducer) -> Drive + Send + Sync + 'a>>,
> {
    gains: Peekable<F>,
    sent: usize,
    is_done: bool,
...
impl<'a, F: ExactSizeIterator<Item = Box<dyn Fn(&Transducer) -> Drive + Send + Sync + 'a>>>
    GainSTMOp<'a, F>
{
    pub fn new(
        gains: F,
