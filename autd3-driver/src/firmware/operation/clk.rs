...
fn mmcm_frac_count_calc(divide: u64, frac: u64) -> u64 {
    ...
    let wf_fall_frac = ((odd_and_frac >= 2) && (odd_and_frac <= 9))
        || ((clkout0_divide_frac == 1) && (clkout0_divide_int == 2));
    let wf_rise_frac = (odd_and_frac >= 1) && (odd_and_frac <= 8);
    ...
    let pm_fall_frac_filtered = pm_fall + pm_rise_frac - ((pm_fall_frac & 0b11111000) | 0b000);

    let drp_regshared: u64 =
        0b110000 | (pm_fall_frac_filtered & 0b111) << 1 | (wf_fall_frac as u64);
    let drp_reg2: u64 = 0b0_000_1_0_0000_000000
        | (clkout0_divide_frac & 0b111) << 12
        | (wf_rise_frac as u64) << 10
        | (dt & 0b111111);
    let drp_reg1: u64 = 0b000_0_000000_000000
        | (pm_rise_frac_filtered & 0b111) << 13
        | (ht_frac & 0b111111) << 6
        | (lt_frac & 0b111111);
