...
pub struct Sender<'a, L: AsyncLink, S: Sleep> {
...
impl<'a, L: AsyncLink, S: Sleep> Sender<'a, L, S> {
    #[doc(hidden)]
    ...
    #[doc(hidden)]
    pub async fn initialize_devices(mut self) -> Result<(), crate::error::AUTDDriverError> {
        self.send(crate::datagram::Nop).await?;
    ...
    #[doc(hidden)]
    pub async fn close(mut self) -> Result<(), crate::error::AUTDDriverError> {
        [
            self.send(Silencer {
