...
impl<'a, L: AsyncLink, S: AsyncSleeper> AsyncSender<'a, L, S> {
    ...
    pub async fn initialize_devices(mut self) -> Result<(), AUTDDriverError> {
        ...
        let r = self._initialize_devices().await;
        if r.is_err()
            && let Ok(list) = self.firmware_version().await
    ...
    async fn _initialize_devices(&mut self) -> Result<(), AUTDDriverError> {
        ...
        self.send((Clear::new(), Synchronize::new())).await
    }
