...
mod internal {
    ...
    pub trait Sender<'a, L, S, T>: Send {
        async fn initialize_devices(self) -> Result<(), AUTDDriverError>;
        async fn firmware_version(self) -> Result<Vec<FirmwareVersion>, AUTDDriverError>;
        async fn close(self) -> Result<(), AUTDDriverError>;
    }
