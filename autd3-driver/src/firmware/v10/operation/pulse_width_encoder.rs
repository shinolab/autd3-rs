...
pub struct PulseWidthEncoderOp<F: Fn(Intensity) -> PulseWidth<ULTRASOUND_PERIOD_COUNT_BITS, u8>> {
    is_done: bool,
    f: F,
}

impl<F: Fn(Intensity) -> PulseWidth<ULTRASOUND_PERIOD_COUNT_BITS, u8>> PulseWidthEncoderOp<F> {
    pub(crate) const fn new(f: F) -> Self {
        Self { is_done: false, f }
    }
}

impl<F: Fn(Intensity) -> PulseWidth<ULTRASOUND_PERIOD_COUNT_BITS, u8> + Send + Sync> Operation
    for PulseWidthEncoderOp<F>
{
    ...
    fn pack(&mut self, _: &Device, tx: &mut [u8]) -> Result<usize, Self::Error> {
        ...
        tx[size_of::<PweMsg>()..]
            ...
            .enumerate()
            .for_each(|(i, dst)| {
                *dst = (self.f)(Intensity(i as u8)).pulse_width();
            });
