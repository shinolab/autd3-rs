...
pub(crate) fn impl_gain_macro(ast: syn::DeriveInput) -> TokenStream {
    ...
    let datagram = quote! {
        impl <#(#lifetimes,)* #(#type_params,)*> DatagramS for #name #ty_generics #where_clause
        {
            ...
            fn operation_generator_with_segment(self, geometry: &Geometry, filter: &DeviceFilter, _: &FirmwareLimits, segment: Segment, transition_mode: Option<TransitionMode>) -> Result<Self::G, Self::Error> {
                Self::G::new(
                    self,
                    geometry,
                    filter,
